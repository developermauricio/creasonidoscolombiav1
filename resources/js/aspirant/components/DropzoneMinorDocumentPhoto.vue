    ...
    <div class="row mt-2">
        <div class="col-12 col-md-6 col-lg-6">
            ...
            <vue2Dropzone class="dropzone upload-logo dropzone-area dz-clickable"
                          ...
                          id="dpz-archives-register-minor-frontal"
                          :options="dropzoneOptionsArchiveFrontal">

            </vue2Dropzone>
            <p style="margin-top: 0.3rem;font-size: 0.9rem; display: none"
               id="text-verify-archive-document-minor-frontal" class="text-danger">La foto del lado frontal es requerida</p>
        </div>
        <div class="col-12 col-md-6 col-lg-6">
            ...
            <vue2Dropzone class="dropzone upload-logo dropzone-area dz-clickable"
                          ...
                          id="dpz-archives-register-minor-back"
                          :options="dropzoneOptionsArchiveBack">
            </vue2Dropzone>
            <p style="margin-top: 0.3rem;font-size: 0.9rem; display: none"
               id="text-verify-archive-document-minor-back" class="text-danger">La foto del lado posterior es requerida</p>
        </div>
    </div>
</template>
...
export default {
    name: "DropzoneMinorDocumentPhoto",
    components: {
        vue2Dropzone,
    },
    data(){
        return{
            ...
            dropzoneOptionsArchiveFrontal: {
                url: '/api/upload-archive-document-minor-photo-frontal',
                // thumbnailWidth: 200,
            ...
            dropzoneOptionsArchiveBack: {
                url: '/api/upload-archive-document-minor-photo-back',
                // thumbnailWidth: 200,
                maxFilesize: 10,  //Tamaño en MB
                maxFiles: 1, // Catidad maxima que se puede subir
                paramName: 'archive',
    ...
    methods:{
        ...
        uploadPhotoFrontal(file){
            ...
            if (name === '' && last_name === ''){
                ...
                this.$toast.error({
                    title: 'Atención',
                    message: 'Para agregar la foto posterior del documento, primero debe ingresar el nombre y apellido del menor de edad',
                    showDuration: 300,
                    hideDuration: 8000,
                    position: 'top right',
                })
        ...
        removedArchiveDropzoneAspirantPhotoFrontal(file, error, xhr) {
            ...
            for (let i = 0; i < this.urlsArchiveArtistFrontal.length; i++)
                if (this.urlsArchiveArtistFrontal[i].uuid === uuid) {
                    ...
                    axios.post('/api/removed-archive-minor-photo-frontal', data)
                        .then(resp => {
                            this.$toast.success({
                                title: '¡Muy bien!',
        ...
        removedArchiveDropzoneAspirantPhotoBack(file, error, xhr) {
            ...
            for (let i = 0; i < this.urlsArchiveArtistBack.length; i++)
                if (this.urlsArchiveArtistBack[i].uuid === uuid) {
                    ...
                    axios.post('/api/removed-archive-minor-photo-back', data)
                        .then(resp => {
                            this.$toast.success({
                                title: '¡Muy bien!',
                                message: 'La foto se quitó correctamente',
